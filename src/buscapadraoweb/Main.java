...
public class Main {
    ...
    public static void buscarPalavras(String url, Map<String, Object> parametros) {
        ...
        int estado = get_string_ref ((String[]) parametros.get("estados"), (String) parametros.get("estado_inicial"));
        ...
        for (int i=0; i<codigoHTML.length(); i++){
            ...
            estado = proximo_estado((char[]) parametros.get("alfabeto"), (int[][]) parametros.get("matriz"), estado, codigoHTML.charAt(i));
            //se o não há transição
            if (estado == -1){
                //pega estado inicial
                estado = get_string_ref((String[]) parametros.get("estados"), (String) parametros.get("estado_inicial"));
                // se o estado anterior foi um estado final
                if (get_string_ref((String[]) parametros.get("estados_finais"), ((String[]) parametros.get("estados"))[estado_anterior]) != -1){
    ...
    public static void main(String[] args) {
        ...
        Map<String, Object> parametros = Map.ofEntries(
            entry("estados", estados),
            entry("alfabeto", alfabeto),
            entry("estado_inicial", estado_inicial),
            entry("estados_finais", estados_finais),
            entry("matriz", matriz)
        );
        
        String[] urls = {
            "https://codigo-postal.org/pt-br/brasil/sc/brusque/",
            "https://cepbrasil.org/santa-catarina/brusque/centro-i/",
            "https://www.cashme.com.br/blog/codigo-postal/"
        };
        ...
        for (int i = 0; i < urls.length; i++) {
            ...
            System.out.println("Palavras encontradas para \"" + url + "\"");
        
            buscarPalavras(url, parametros);
            
            if (urls.length-1 <= i) break;
